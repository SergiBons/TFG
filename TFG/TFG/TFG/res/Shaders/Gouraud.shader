...
uniform mat4 u_NormalMatrix;	// “the transpose of the inverse of the Modelu_ViewMatrix.” 
...
// --- L66- Calcul variables previes.
    //mat4 u_NormalMatrix = transpose(inverse(u_ViewMatrix * u_ModelMatrix));
    vec3 N = vec3(u_NormalMatrix * vec4(in_Normal,1.0));
    N = normalize(N);
    vec3 vertexPV = vec3(u_ViewMatrix * u_ModelMatrix * vec4(in_Vertex,1.0));
...
// --- L102- Multiples llums
	...
	if (LightSource[i].sw_light) {
		...
		if (LightSource[i].restricted) 
                  { spotDirectionPV = vec3(u_NormalMatrix * vec4(LightSource[i].spotDirection,1.0));
...
// --- L157- Transformacio de Visualitzacio amb Matriu Projeccio (u_ProjectionMatrix), Matriu Càmera (u_ViewMatrix) i Matriu TG (u_ModelMatrix)
...
if (textur) {	// Intensitat amb textura
		vec4 colorT = texture(u_Texture1,VertexTexCoord);
