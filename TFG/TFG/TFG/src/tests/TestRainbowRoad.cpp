...
namespace test {
    ...
    test::TestRainbowRoad::TestRainbowRoad()
    ...
    void test::TestRainbowRoad::OnUpdate(float deltaTime)
    {
    }

    void test::TestRainbowRoad::OnRender()
    {
        ...
        for (int i = 0; i<m_sizeSlide; i++)
            for (int j = 0; j < 3; j++)
            {
                ...
                //model = glm::rotate(model, glm::radians(-55.0f), m_TranslationA);
                model = glm::translate(model, m_Translation);
                //model = glm::translate(model, glm::vec3(1.0f * i, -2.5f, 0.0f));
    ...
    void test::TestRainbowRoad::OnImGuiRender()
    {
